...
io.on('connection', function (socket) {
    ...
    if (players.length === 1) selectCurrentPlayer()
    ...
    socket.on('stdin', (stdin: string) => {
        ...
        if (currentPlayerRemainingInput <= 0) selectCurrentPlayer()
    })
...
const selectCurrentPlayer = () => {
    if (players.length < 1) currentPlayer = undefined
    else if (players.length == 1) currentPlayer = players[0]
    else if (currentPlayer == undefined) currentPlayer = players[0]
    else currentPlayer = findPlayerOtherThan(currentPlayer)
...
const findPlayerOtherThan = (player: string): string => {
